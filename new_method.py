...
default_image = os.getenv("ATTWARP_IMAGE", "examples/image.png")
default_query = "On the right desk, what is to the left of the laptop?"
default_output_dir = Path(os.getenv("ATTWARP_OUTPUT_DIR", "outputs/attwarp"))
...
def example_workflow():
    ...
    for i, (image_path, query) in enumerate(zip(images, queries)):
        ...
        np.save(default_output_dir / f"attention_map_{i}.npy", att_map_np)
        ...
        if isinstance(mota_mask, Image.Image):
            ...
            mota_mask_gray.save(default_output_dir / "mota_mask_gray.png")
            mota_mask_np = np.array(mota_mask_gray)
            np.save(default_output_dir / "mota_mask.npy", mota_mask_np)
        ...
        if isinstance(merged_image, Image.Image):
            ...
            output_path = default_output_dir / "mota_mask_image.png"
            cv2.imwrite(str(output_path), merged_img_np)
...
def main():
    ...
    # Required arguments
    parser.add_argument("--image", required=False, default=default_image, help="Path to input image file")
    parser.add_argument("--attention-map", required=False, default=str(default_output_dir / "mota_mask.npy"), help="Path to attention map .npy file")
    ...
    base_output_dir = str(default_output_dir / "output_runs")
